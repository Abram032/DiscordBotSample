...
{
    public class SubtractCurrency : ModuleBase<SocketCommandContext>
    {
        public IConfiguration Configuration { get; set; }
        public SubtractCurrency(IConfiguration configuration)
        {
            Configuration = configuration;
        }

        [Command("subtract")]
        [Summary("Subtracts specified amount of currency from user.")]
        public async Task ExecuteAsync(int amount, SocketUser user = null) 
        {
            ...
            if(amount < 0) {
                embed.WithDescription($"You can't subtract negative amount of {currencyName}!");
            ...
            using (var _dbContext = new BotContext())
            {
                ...
                _user.Points -= amount;
                embed.WithDescription($"Subtracted **{amount}** {currencyName} from {user.Mention}.");
